...
async function tongji() {
  ...
  for (let e of 复习时间列表) {
    ...
    let 上次复习日期 =
      new Date(e.上次复习时间).getFullYear() +
      "-" +
      (new Date(e.上次复习时间).getMonth() + 1) +
      "-" +
      new Date(e.上次复习时间).getDate();
    let 当天日期 =
  ...
  统计数值.ignore = await sql(
    `select count(*) as count from blocks where id in (select block_id from attributes where name='custom-randomNoteType' and value = 'ignore')`
  ).then((e) => {
    return e[0].count;
  });
}
